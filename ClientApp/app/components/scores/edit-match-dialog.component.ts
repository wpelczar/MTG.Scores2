...
function matchFormValidator(c: AbstractControl): { [key: string]: boolean } | null {
  let player1Control = c.get('selectedPlayer1');
  let player2Control = c.get('selectedPlayer2');
  let player1Score = c.get('player1Score');
  let player2Score = c.get('player2Score');
